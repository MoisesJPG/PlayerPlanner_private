...
import { InventoryMat } from "../entity/InventoryMatEntity.js";
...
class CharacterProxy {
     ...
     */
    static get(key) {
        return CharacterList[typeof key === "number" ? CharacterMap.get(key) : key];
    }
    /**
    ...
    static getMaterials() {
        ...
        for (let characterName in CharacterList) {
            const character = CharacterProxy.get(characterName);
...
/**
 * @typedef {typeof CharacterProxy & typeof CharacterList} CharacterType
 * @type {CharacterType}
**/
const Character = new Proxy(CharacterList, {
    get(target, prop) {
        if (prop in CharacterProxy) return CharacterProxy[prop];
